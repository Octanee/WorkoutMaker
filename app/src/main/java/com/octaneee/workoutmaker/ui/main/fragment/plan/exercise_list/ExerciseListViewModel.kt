...
class ExerciseListViewModel @ViewModelInject constructor(
    ...
    fun saveSets(dataSet: List<Series>, trainingId: Long, exerciseId: Long) =
    ...
    fun deleteExercise(item: Exercise) = viewModelScope.launch(Dispatchers.IO) {
        exerciseRepository.delete(item)
    }

    fun insertExercise(item: Exercise): Long = runBlocking {
        exerciseRepository.insert(item)
    }
}